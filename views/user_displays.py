...
class User_displays:
    '''This class display the selection menus of choice, find chosen data in database and display them'''
    ...
    def main_choice_menu(cls):
        ...
        list_headers = ["       Choix N°     ","     Faite votre choix     "]
        rows =[ ]
        ...
        while 1:
            try:
                ...
                if str(choice.upper()) == "Q":
                    ...
                    choice = int(choice)
                if choice in range(1,len(display_constants.main_choice_text)+1):
                    print("Vous avez choisi '{}': ".format(display_constants.main_choice_text[choice-1].strip("\t")))
                    time.sleep(1)
                    return choice
                    break
                else:
    ...
    def products_choice_menu(cls, category):
        ...
        products, counter = cls.db_request.find_products(queries_constants.FIND_ALL_PRODUCTS_BY_CATEGORIE, None, category)
    ...
    def substitutes_choice_menu(cls, prod_choice_id):
        ...
        product, counter = cls.db_request.find_products(queries_constants.FIND_PRODUCTS_BY_ID, prod_choice_id, cls.category)
        cls.db_request.reset_offset()
        substitutes, counter = cls.db_request.find_products(queries_constants.FIND_SUBSTITUTES_BY_ID, prod_choice_id, cls.category)
        list_rows = ['Index']+[row for row in constants.fr_food_informations.keys()]
        while 1:
            list_ids = []
            ...
            cls.display_products( [product[0][:6] + product[0][9:]] , (list_rows[:6]+list_rows[9:]))
            cls.separate_text("Substitute")
            cls.display_products(substitutes, (list_rows[:6]+list_rows[9:]))
            try:
                choice = input(display_constants.colours["black"] + display_constants.colours["on_cyan"] + display_constants.messages["Pagination_info"]+"\n"+ display_constants.colours["default"] + display_constants.messages['Substitute_choice'])
                if str(choice.upper()) == "Q":
                    choice = 0
                    return choice
                    break
    ...
    def comparison_and_save_menu(cls,substitute_id):
        ...
        cls.db_request.reset_offset()
        product_details, counter = cls.db_request.find_products(queries_constants.FIND_PRODUCTS_BY_ID, cls.prod_choice_id, cls.category)
        substitute_details, counter = cls.db_request.find_products(queries_constants.FIND_PRODUCTS_BY_ID, substitute_id, cls.category)

        list_rows_names = ['Index']+[row for row in constants.fr_food_informations.keys()]
        columns_name = ["  ", "Aliment à substituer", "Aliment de substitution"]
        while 1:
            system('cls||clear')
            cls.message("Welcome")
            ...
            while i < len(substitute_details[0]):
                list_rows.append([list_rows_names[i], product_details[0][i], substitute_details[0][i]])
                i = i+1
            cls.display_products(list_rows, columns_name)
            try:
                choice = input(display_constants.colours["black"] + display_constants.colours["on_cyan"] + display_constants.messages["Pagination_info"]+"\n"+ display_constants.colours["default"] + display_constants.messages['Save_substitute'])
                if str(choice.upper()) == "Q":
                    choice = 0
                    return choice
                    break
                    ...
                    if choice.upper() == "OUI":
                        try:
                            cls.db_request.save_substitute(queries_constants.SAVE_SUBSTITUTE,cls.prod_choice_id, substitute_id)
                            cls.message("Confim_saved","black","on_yellow",cls.prod_choice_id, substitute_id )
                        except:
    ...
    def favorites(cls):
        cls.db_request.reset_offset()
        product, substitute, counter = cls.db_request.find_favorite(queries_constants.FIND_FAVORITE_BY_ID, 1)
        counter = counter -1
        ...
        while 1:
            ...
            while i < len(list_rows_names):
                list_rows.append([list_rows_names[i], product[0][i], substitute[0][i]])
                i = i+1
            cls.display_products(list_rows, columns_name)
            try:
                choice = input(display_constants.colours["black"] + display_constants.colours["on_cyan"] + display_constants.messages["Pagination_info"]+"\n"+ display_constants.colours["default"] + display_constants.messages['Favorites'])
                choice = str(choice)
                if choice.upper() == "Q":
                    return choice
                    break
    ...
    @classmethod
    def message(cls, message, color ="black", background_color = "on_cyan",  *params):
        text_message = print("\n"+display_constants.colours["bold"]+
        display_constants.colours[color]+
        display_constants.colours[background_color]+
        display_constants.messages[message].format(*params)+
        display_constants.colours["default"]+
        "\n")
        return text_message
    ...
    def system_info(cls, message, *param):
        text_message = print(display_constants.del_line+
        display_constants.colours["black"]+
        display_constants.colours["on_yellow"]+
        display_constants.colours["blink"]+
        display_constants.system_info[message].format(*param)+
        display_constants.return_to_preview_line+
        display_constants.colours["default"]
        )
        return text_message
